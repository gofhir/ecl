...
altidentifier : ((qm altidentifierschemealias HASH altidentifiercodewithinquotes qm) | (altidentifierschemealias HASH altidentifiercodewithoutquotes)) (ws PIPE ws term ws PIPE)?;
altidentifierschemealias : alpha (dash | alpha | integervalue)*;
altidentifiercodewithinquotes : anynonescapedchar+;
altidentifiercodewithoutquotes : (alpha | digit | dash | PERIOD | UNDERSCORE)+;
wildcard : ASTERISK;
...
top : (EXCLAMATION EXCLAMATION GREATER_THAN);
bottom : (EXCLAMATION EXCLAMATION LESS_THAN);
...
eclattribute : (LEFT_BRACE cardinality RIGHT_BRACE ws)? (reverseflag ws)? eclattributename ws ((expressioncomparisonoperator ws subexpressionconstraint) | (numericcomparisonoperator ws HASH numericvalue) | (stringcomparisonoperator ws (concretestring | concretestringset)) | (booleancomparisonoperator ws booleanvalue));
...
idcomparisonoperator : EQUALS | (EXCLAMATION EQUALS);
...
descriptionidkeyword : ((CAP_I | I)|(CAP_I | I)) ((CAP_D | D)|(CAP_D | D));
descriptionid : sctid;
descriptionidset : LEFT_PAREN ws descriptionid (mws descriptionid)* ws RIGHT_PAREN;
...
concretestringset : LEFT_PAREN ws concretestring (mws concretestring)* ws RIGHT_PAREN;
concretestring : qm concretestringcharacters qm;
concretestringcharacters : (anynonescapedchar | escapedwildchar)+;
